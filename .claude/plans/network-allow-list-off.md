...
Flag name: `SBXAGENT_NETWORK_ALLOWLIST` (`true` default, `false` = open),
mirroring the user's mental model ("allow list on/off") and the
`SBXAGENT_` prefix of `SBXAGENT_LITE`, which is also exported into the
sandbox.
...
# Stacked by scripts/sbxagent with --kit when SBXAGENT_NETWORK_ALLOWLIST=false.
...
environment:
  variables:
    SBXAGENT_NETWORK_ALLOWLIST: "false"

agentInstructions:
  content: |
    ## Network
    This sandbox was created with SBXAGENT_NETWORK_ALLOWLIST=false: the
...
- Next to `SBXAGENT_LITE` (`scripts/sbxagent:69-71`): comment + default
  `SBXAGENT_NETWORK_ALLOWLIST="${SBXAGENT_NETWORK_ALLOWLIST:-true}"`.
- In `compute_state_mounts()` (`scripts/sbxagent:127-147`), follow the
  ...
  ```bash
  case "${SBXAGENT_NETWORK_ALLOWLIST}" in
  true) NETWORK_KIT="" ;;
  false) NETWORK_KIT="${REPO}/mixins/open-network" ;;
  *) die "SBXAGENT_NETWORK_ALLOWLIST must be true or false, not '${SBXAGENT_NETWORK_ALLOWLIST}'" ;;
  esac
  ```
...
- `SBXAGENT_NETWORK_ALLOWLIST=false run_claude "${WORK_B}" create` →
...
- `SBXAGENT_NETWORK_ALLOWLIST=bogus reject_without_call ... create`, and
  `name` still works with a bogus value (matches the LITE test).
- Add `SBXAGENT_NETWORK_ALLOWLIST=false` to the `.env` heredoc and assert the
  `--kit` operand appears; assert a real env var (`=true`) beats `.env`.

### 5. `tests/toolchain_test.sh`

Add a check that branches on `"${SBXAGENT_NETWORK_ALLOWLIST:-true}"` the way
...
- `docs/toolchain.md:35`: "Network access is an allowlist, not the open
  ...
  `SBXAGENT_NETWORK_ALLOWLIST=false`", and note it under "Rebuild after kit
...
3. On the host (user runs; needs sandboxd): in a scratch project,
   `SBXAGENT_NETWORK_ALLOWLIST=false sbxclaude create`, then
